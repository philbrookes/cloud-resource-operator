...
func New(mgr manager.Manager) (*PostgresReconciler, error) {
	...
	providerList := []providers.PostgresProvider{openshift.NewOpenShiftPostgresProvider(client, clientSet, logger), aws.NewAWSPostgresProvider(client, logger), azure.NewAzurePostgresProvider(client, logger)}
